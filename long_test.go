...
// This test applies gosl to all Go files under -root.
// To test specific files provide a list of comma-separated
// filenames via the -files flag: go test -files=gosl.go .
...
func gosl(fset *token.FileSet, filename string, src *bytes.Buffer) error {
...
func testFile(t *testing.T, b1, b2 *bytes.Buffer, filename string) {
	...
	// gosl file
	if err = gosl(fset, filename, b1); err != nil {
		t.Errorf("1st gosl failed: %v", err)
		return
	}
	...
	// gosl result again
	if err = gosl(fset, filename, b2); err != nil {
		t.Errorf("2nd gosl failed: %v", err)
		return
	}

	// the first and 2nd result should be identical
	if !bytes.Equal(b1.Bytes(), b2.Bytes()) {
		// A known instance of gosl not being idempotent
		// (see Issue #24472)
		if strings.HasSuffix(filename, "issue22662.go") {
			t.Log("known gosl idempotency bug (Issue #24472)")
			return
		}
		t.Errorf("gosl %s not idempotent", filename)
	}
}
