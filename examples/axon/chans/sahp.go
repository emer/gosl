...
type SahpParams struct {
	...
	// [def: 5,10] time constant for integrating Ca across theta cycles
	CaTau float32 `def:"5,10" desc:"time constant for integrating Ca across theta cycles"`

	// [def: 0.8] integrated Ca offset (threshold) for infinite time N gating function -- where the gate is at 50% strength
	...
	// [def: 0.02] slope of the infinite time logistic gating function
	Slope float32 `def:"0.02" desc:"slope of the infinite time logistic gating function"`

	// [def: 1] maximum slow rate time constant in msec for activation / deactivation.  The effective Tau is much slower -- 1/20th in original temp, and 1/60th in standard 37 C temp
